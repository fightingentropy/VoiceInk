...
// Converts spoken punctuation phrases ("comma", "open paren", "at the rate", ...)
// into their symbols, with context guards so prose like "the polka dot dress" is untouched.
...
enum SpokenPunctuationFormatter {
    ...
    private struct PhraseRule {
        ...
        var requiresEmailContext = false
        var requiresAdjacentDigitContext = false
    }
    ...
    /// `frontmostAppContext` is a haystack of the frontmost app's name and
    /// bundle identifier; some apps (e.g. Cursor, ChatGPT) have bundle IDs
    /// that don't contain their product name.
    static func apply(_ text: String, frontmostAppContext: String? = nil) -> String {
        guard !text.isEmpty else { return text }

        let isAtSignPunctuationApp = self.isAtSignPunctuationApp(context: frontmostAppContext)
        ...
        while index < tokens.count {
            if let match = self.matchRule(in: tokens, at: index, isAtSignPunctuationApp: isAtSignPunctuationApp) {
    ...
    private static func isAtSignPunctuationApp(context: String?) -> Bool {
    ...
    private static func makeRules() -> [PhraseRule] {
        self.rules(
            ...
            self.rules(
                symbol: "%",
                spacing: .rightAttached,
                phrases: ["percent sign", "percentage sign"]
            ) +
            self.rules(
                symbol: "%",
                spacing: .rightAttached,
                phrases: ["percent"],
                requiresAdjacentDigitContext: true
            ) +
            self.rules(
                symbol: "$",
                spacing: .leftAttached,
            ...
            self.rules(
                symbol: "#",
                spacing: .noSpaceAround,
                phrases: ["hash sign", "hashtag", "pound sign", "number sign"]
            ) +
            self.rules(
                symbol: "#",
                spacing: .noSpaceAround,
    ...
    private static func rules(
        ...
        requiresEmailContext: Bool = false,
        requiresAdjacentDigitContext: Bool = false
    ) -> [PhraseRule] {
        phrases.compactMap { phrase in
            ...
            return PhraseRule(
                ...
                requiresEmailContext: requiresEmailContext,
                requiresAdjacentDigitContext: requiresAdjacentDigitContext
            )
        }
    }
    ...
    private static func matchRule(
        ...
        for rule in candidates {
            ...
            if matched,
               ...
               !rule.requiresAtSignPunctuationApp || isAtSignPunctuationApp,
               !rule.requiresEmailContext ||
               self.hasEmailContext(in: tokens, endIndex: cursor),
               !rule.requiresAdjacentDigitContext ||
               self.hasAdjacentDigitContext(in: tokens, startIndex: index, endIndex: cursor)
            {
                return (rule, cursor)
            }
        }
    ...
    /// True when the tokens after an "at the rate" phrase look like an
    /// email/handle tail (e.g. "example dot com", "gmail.com"), so prose like
    /// "growing at the rate of 5 percent" stays untouched.
    private static func hasEmailContext(in tokens: [Token], endIndex: Int) -> Bool {
        var cursor = endIndex
        var inspected = 0
        while cursor < tokens.count, inspected < 3 {
            let token = tokens[cursor]
            if token.isHorizontalWhitespaceText {
                cursor += 1
                continue
            }
            if token.normalizedWord == "dot" {
                return true
            }
            if case let .text(text) = token, text.contains(where: { $0 == "." || $0 == "@" }) {
                return true
            }
            inspected += 1
            cursor += 1
    ...
    private static func hasAdjacentDigitContext(in tokens: [Token], startIndex: Int, endIndex: Int) -> Bool {
        if let previous = self.digitContextToken(before: startIndex, in: tokens),
           previous.text?.last?.isASCIIDigit == true
        {
            return true
        }
        if let next = self.significantToken(atOrAfter: endIndex, in: tokens),
           next.text?.first?.isASCIIDigit == true
        {
            return true
        }
        return false
    ...
    /// Walks back over spoken commas so "50 comma percent" still counts as digit-adjacent.
    private static func digitContextToken(before index: Int, in tokens: [Token]) -> Token? {
        var cursor = index
        while let tokenIndex = self.significantTokenIndex(before: cursor, in: tokens) {
            if tokens[tokenIndex].normalizedWord == "comma" {
                cursor = tokenIndex
                continue
            }

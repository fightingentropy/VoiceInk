...
// Converts spoken "/command" and "@mention" phrases into literal tokens, with
// reject-lists, lead-in word checks, and app-aware relaxed rules.
...
enum DictationLiteralFormatter {
    ...
    private static let mentionRejectedTokens: Set<String> = [
        "a", "an", "airport", "along", "and", "are", "at", "be", "been",
        "breakfast", "brunch", "but", "class", "dinner", "for", "her", "him",
        "his", "home", "hotel", "house", "in", "is", "it", "its", "lunch", "me",
        "meeting", "my", "night", "noon", "of", "office", "on", "or", "our",
        "place", "restaurant", "school", "shop", "store", "that", "the", "them",
        "these", "they", "this", "those", "to", "today", "tomorrow", "us",
        "was", "we", "were", "with", "work", "yesterday", "you", "your",
    ]
    ...
    private static let explicitMentionLeadInWords: Set<String> =
        relaxedMentionLeadInWords.union(["also", "and", "just", "please", "then"])
    ...
    static func apply(_ text: String, frontmostAppContext: String? = nil) -> String {
        ...
        return self.applyMentionFormatting(commandFormatted, frontmostAppContext: frontmostAppContext)
    }
    ...
    static func applyMentionFormatting(_ text: String, frontmostAppContext: String? = nil) -> String {
        guard !text.isEmpty,
        ...
        guard self.isRelaxedMentionApp(context: frontmostAppContext) else {
    ...
    static func applyTerminalLiteralAutocompleteSpacing(_ text: String, frontmostAppContext: String? = nil) -> String {
        ...
        if self.isSlashCommandAutocompleteApp(context: frontmostAppContext),
           self.matchesWholeString(withoutTrailingWhitespace, regex: self.standaloneSlashCommandRegex)
        {
            return withoutTrailingWhitespace
        }

        if self.isRelaxedMentionApp(context: frontmostAppContext),
    ...
    private static func replacingSlashCommandMatches(
        ...
        for match in matches.reversed() {
            ...
            if kind == .spoken,
               !self.hasLeadInContext(
                   in: source,
                   matchLocation: match.range.location,
                   leadInWords: self.slashCommandSpokenLeadInWords
               )
            {
                continue
            }
    ...
    /// True at the start of the text/sentence or when the preceding word is in
    /// `leadInWords`, so mid-prose phrases stay untouched.
    private static func hasLeadInContext(in text: NSString, matchLocation: Int, leadInWords: Set<String>) -> Bool {
        ...
        return leadInWords.contains(String(previousWord).lowercased())
    }
    ...
    private static func replacingMentionMatches(
        ...
        for match in matches.reversed() {
            ...
            if kind == .explicit,
               self.isTagOrMentionMatch(source.substring(with: match.range)),
               !self.hasLeadInContext(
                   in: source,
                   matchLocation: match.range.location,
                   leadInWords: self.explicitMentionLeadInWords
               )
            {
                continue
            }
            if kind == .relaxed,
               !self.hasLeadInContext(
                   in: source,
                   matchLocation: match.range.location,
                   leadInWords: self.relaxedMentionLeadInWords
               )
            {
                continue
            }
    ...
    /// The "tag Name" / "mention Name" phrases are everyday prose, unlike
    /// "at sign" / "at the rate", so they get the lead-in gate on top of the
    /// shared name validation.
    private static func isTagOrMentionMatch(_ matchedText: String) -> Bool {
        let lowercased = matchedText.lowercased()
        return lowercased.hasPrefix("tag") || lowercased.hasPrefix("mention")
    }

    private static func isRelaxedMentionApp(context: String?) -> Bool {
    ...
    private static func isSlashCommandAutocompleteApp(context: String?) -> Bool {
        guard let haystack = context?.lowercased() else { return false }
